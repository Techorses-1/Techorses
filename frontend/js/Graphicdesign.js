...
document.addEventListener('DOMContentLoaded', function () {
  const filterButtons = document.querySelectorAll('.graphic-design-section .filter-btn');
  const techItems = document.querySelectorAll('.graphic-design-section .tech-item');

  // Since there's only one category ("All"), we'll just show all items by default
  function initFilters() {
    filterButtons.forEach(button => {
      button.addEventListener('click', function () {
        filterButtons.forEach(btn => btn.classList.remove('active'));
        this.classList.add('active');
        // Show all items regardless of filter since only "All" exists
    ...
    document.querySelector('.graphic-design-section .filter-btn[data-category="all"]').classList.add('active');
