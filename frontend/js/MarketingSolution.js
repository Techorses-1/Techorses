...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (serviceForm) {
    serviceForm.addEventListener('submit', async function (e) {
      ...
      try {
        const res = await fetch('https://api.techorses.com/api/service-inquiry', {
          method: 'POST',
...
document.addEventListener('DOMContentLoaded', function () {
  const filterButtons = document.querySelectorAll('.marketing-solutions-section .filter-btn');
  const techItems = document.querySelectorAll('.marketing-solutions-section .tech-item');
  const sliderContainer = document.querySelector('.marketing-solutions-section .mobile-tech-slider-container');
  ...
  function initDesktopFilters() {
    ...
    document.querySelector('.marketing-solutions-section .filter-btn[data-category="designing"]').classList.add('active');
    filterItems('designing');
  }

  function initMobileSlider() {
    const track = document.querySelector('.marketing-solutions-section .slider-track');
    const dotsContainer = document.querySelector('.marketing-solutions-section .slider-dots');

    track.innerHTML = '';
    dotsContainer.innerHTML = '';

    // Create 5 static dots (3rd one active)
    for (let i = 0; i < 5; i++) {
      const dot = document.createElement('div');
      dot.className = 'slider-dot';
      if (i === 2) dot.classList.add('active'); // 3rd dot is active
    ...
    const slides = document.querySelectorAll('.marketing-solutions-section .slider-slide:not(.empty)');

    // Start with designing as default
    let currentIndex = Array.from(filterButtons).findIndex(btn => btn.dataset.category === 'webtools');
    ...
    function positionSlides() {
      ...
      document.querySelectorAll('.marketing-solutions-section .slider-slide').forEach(slide => {
    ...
    function goToSlide(index) {
      ...
      const activeSlide = document.querySelector(`.marketing-solutions-section .slider-slide[data-index="${index}"]`);
    ...
    function updateSlider() {
      ...
      const dots = document.querySelectorAll('.marketing-solutions-section .slider-dot');
      dots.forEach((dot, i) => {
  ...
  window.addEventListener('resize', function () {
    if (window.innerWidth <= 768) {
      if (!document.querySelector('.marketing-solutions-section .slider-track').children.length) {
        initMobileSlider();
      }
    } else {
      initDesktopFilters();
