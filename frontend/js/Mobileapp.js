...
document.addEventListener('DOMContentLoaded', function () {
  const filterButtons = document.querySelectorAll('.mobile-app-section .filter-btn');
  const techItems = document.querySelectorAll('.mobile-app-section .tech-item');
  const sliderContainer = document.querySelector('.mobile-app-section .mobile-tech-slider-container');
  ...
  function initDesktopFilters() {
    ...
    document.querySelector('.mobile-app-section .filter-btn[data-category="crossplatform"]').classList.add('active');
    filterItems('crossplatform');
  }

  function initMobileSlider() {
    const track = document.querySelector('.mobile-app-section .slider-track');
    const dotsContainer = document.querySelector('.mobile-app-section .slider-dots');

    track.innerHTML = '';
    dotsContainer.innerHTML = '';

    // Create 7 static dots (middle one always active)
    for (let i = 0; i < 7; i++) {
      const dot = document.createElement('div');
      dot.className = 'slider-dot';
      if (i === 3) dot.classList.add('active'); // Middle dot is active
    ...
    const slides = document.querySelectorAll('.mobile-app-section .slider-slide:not(.empty)');

    // Start with crossplatform as default
    ...
    function positionSlides() {
      ...
      document.querySelectorAll('.mobile-app-section .slider-slide').forEach(slide => {
    ...
    function goToSlide(index) {
      ...
      const activeSlide = document.querySelector(`.mobile-app-section .slider-slide[data-index="${index}"]`);
    ...
    function updateSlider() {
      ...
      const dots = document.querySelectorAll('.mobile-app-section .slider-dot');
      dots.forEach((dot, i) => {
        dot.classList.toggle('active', i === 3); // Keep middle dot active
      });
    }
  ...
  window.addEventListener('resize', function () {
    if (window.innerWidth <= 768) {
      if (!document.querySelector('.mobile-app-section .slider-track').children.length) {
        initMobileSlider();
      }
    } else {
      initDesktopFilters();
