...
document.addEventListener('DOMContentLoaded', function () {
  const filterButtons = document.querySelectorAll('.uiux-design-section .filter-btn');
  const techItems = document.querySelectorAll('.uiux-design-section .tech-item');
  const sliderContainer = document.querySelector('.uiux-design-section .mobile-tech-slider-container');
  ...
  function initDesktopFilters() {
    ...
    document.querySelector('.uiux-design-section .filter-btn[data-category="designing"]').classList.add('active');
    filterItems('designing');
  }

  function initMobileSlider() {
    const track = document.querySelector('.uiux-design-section .slider-track');
    const dotsContainer = document.querySelector('.uiux-design-section .slider-dots');
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (i === 2) dot.classList.add('active'); // Middle dot is active (now index 2 for 5 dots)
    ...
    const slides = document.querySelectorAll('.uiux-design-section .slider-slide:not(.empty)');

    // Start with designing as default
    let currentIndex = Array.from(filterButtons).findIndex(btn => btn.dataset.category === 'frontend');
    ...
    function positionSlides() {
      ...
      document.querySelectorAll('.uiux-design-section .slider-slide').forEach(slide => {
    ...
    function goToSlide(index) {
      ...
      const activeSlide = document.querySelector(`.uiux-design-section .slider-slide[data-index="${index}"]`);
    ...
    function updateSlider() {
      ...
      const dots = document.querySelectorAll('.uiux-design-section .slider-dot');
      dots.forEach((dot, i) => {
        dot.classList.toggle('active', i === 2); // Keep middle dot active
      });
    }
  ...
  window.addEventListener('resize', function () {
    if (window.innerWidth <= 768) {
      if (!document.querySelector('.uiux-design-section .slider-track').children.length) {
        initMobileSlider();
      }
    } else {
      initDesktopFilters();
